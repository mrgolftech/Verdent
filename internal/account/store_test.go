...
func TestFileStoreRoundTripKeepsSecretOffRuntimeJSON(t *testing.T) {
	...
	want := []Credential{{ID:"a",Label:"one",Token:"secret",DeviceID:"dev",TeamID:"0",ProxyURL:"http://127.0.0.1:8080"}}
	...
	if len(got) != 1 || got[0].Token != "secret" || got[0].ProxyURL != want[0].ProxyURL { t.Fatalf("bad round trip: %#v", got) }
}
