...
func TestFileStoreRoundTripKeepsSecretOffRuntimeJSON(t *testing.T) {
	...
	want := []Credential{{ID:"a",Label:"one",Token:"secret",RefreshToken:"refresh-secret",TokenExpiresAt:1900000000,DeviceID:"dev",TeamID:"0",ProxyURL:"http://127.0.0.1:8080"}}
	...
	if len(got) != 1 || got[0].Token != "secret" || got[0].RefreshToken != "refresh-secret" || got[0].TokenExpiresAt != 1900000000 || got[0].ProxyURL != want[0].ProxyURL { t.Fatalf("bad round trip: %#v", got) }
}
