...
func TestStreamDecoderTextThinkingToolAndUsage(t *testing.T) {
	...
	if len(got) != 11 {
		t.Fatalf("expected 11 events, got %d: %#v", len(got), got)
	}
	if got[1].Type != EventUsage || got[1].Usage.InputTokens != 12 {
		t.Fatalf("bad input usage: %#v", got[1])
	}
	if got[2].Type != EventThinking || got[2].Text != "plan" {
		t.Fatalf("bad thinking start: %#v", got[2])
	}
	if got[4].Type != EventToolStart || got[4].ToolName != "read_file" {
		t.Fatalf("bad tool start: %#v", got[4])
	}
	if got[5].ArgumentDelta != "{\"path\":\"/tmp/a" {
		t.Fatalf("bad first tool delta: %q", got[5].ArgumentDelta)
	}
	if got[8].Type != EventTextDelta || got[8].Text != "done" {
		t.Fatalf("bad text: %#v", got[8])
	}
	if got[9].Type != EventUsage || got[9].Usage.OutputTokens != 7 {
		t.Fatalf("bad output usage: %#v", got[9])
	}
	if got[10].Type != EventMessageEnd {
		t.Fatalf("missing message end: %#v", got[10])
	}
}
