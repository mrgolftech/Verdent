...
func (s *Server) handleChatCompletions(w http.ResponseWriter,r *http.Request) {
	...
	if in.Stream { s.streamChat(w,resp,a.Credential.ID,id,in.Model,created,in.StreamOptions!=nil && in.StreamOptions.IncludeUsage); return }
	s.collectChat(w,resp,a.Credential.ID,id,in.Model,created)
}

func (s *Server) collectChat(w http.ResponseWriter,resp *http.Response,accountID,id,model string,created int64) {
	...
	for {
		...
		for _,ev:=range events {
			if ev.Type==protocol.EventError {
				code:=s.recordUpstreamFailure(accountID,0,ev.Err,"")
				writeAPIError(w,http.StatusBadGateway,code,ev.Err); return
			}
...
func (s *Server) streamChat(w http.ResponseWriter,resp *http.Response,accountID,id,model string,created int64,includeUsage bool) {
	...
	for {
		...
		for _,ev:=range events {
			if ev.Type==protocol.EventError {
				code:=s.recordUpstreamFailure(accountID,0,ev.Err,"")
				writeSSEError(w,code,ev.Err); flusher.Flush(); return
			}
...
func (s *Server) handleUpstreamHTTPError(w http.ResponseWriter,accountID string,resp *http.Response) {
	...
	code:=s.recordUpstreamFailure(accountID,resp.StatusCode,raw,resp.Header.Get("Retry-After"))
	if code=="upstream_error" { code="upstream_"+strconv.Itoa(resp.StatusCode) }
