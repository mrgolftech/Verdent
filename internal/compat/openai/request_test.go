...
func TestToCanonicalToolsAndHistory(t *testing.T) {
	req:=ChatRequest{
		Model:"gpt-5.6-luna-free",MaxCompletionTokens:4096,ReasoningEffort:"high",
		Messages:[]Message{
	...
	if got.MaxTokens!=4096 || got.Effort!="high" || len(got.System)!=1 || len(got.Tools)!=1 { t.Fatalf("bad canonical request: %#v",got) }
